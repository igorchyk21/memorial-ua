...
interface Props {
    ...
    onClickEdit:()=>void;
    audioStatus:HERO_VIDEO_STAT;
    disabled?:boolean;
    showDelete?:boolean;
    showActions?:boolean;
}
  
const HeroAudioToolbar = ({onClickDelete,onClickStatus,onClickEdit,audioStatus,disabled,showDelete,showActions}:Props) => {
    ...
    return (<> 
        ...
        (<Button className="btn-icon ms-1 rounded-circle p-0" 
            ...
            onClick={async ()=>onClickEdit()}>
                <i className="ci-volume-2"/></Button>)}


        {[HERO_VIDEO_STAT.PENDING, HERO_VIDEO_STAT.REJECT].includes(audioStatus) && showActions &&
        (<Button className="btn-icon ms-1 rounded-circle p-0" 
            variant="success"
            disabled={disabled}
            style={{width:24, height:24}}
        ...
        {[HERO_VIDEO_STAT.PENDING, HERO_VIDEO_STAT.ACTIVE].includes(audioStatus) && showActions &&
...
export default HeroAudioToolbar;
